...
<table border="0" cellspacing="1"  id="STable" style="width:700px;">
	...
	<tr><td>所在服务器</td>
	<td>
	<select id="remote_ecs_dom" name="remote_ecs" onchange="remote_ecs_change()">
	<option value="1">阿里云ECS</option>
	<option value="2">其它服务器</option>
	</select>
	<?php if(isset($_POST['remote_ecs'])) {?>
	<script>G('remote_ecs_dom').value = '<?php echo $_POST['remote_ecs'];?>';</script>
	<?php }?>
	</td>
	<td><p> &nbsp; <font class="red">*</font> 选择您当前所在服务器位置</p></td>
	</tr>

	<tr><td>Host</td>
	<td><input type="text" id="remote_ip" name="remote_ip" class="input_text" value="<?php echo isset($_POST['remote_ip']) ? $_POST['remote_ip'] : 'oss-internal.aliyuncs.com';?>" /></td>
...
<script>
G('remote_ecs_dom').value = (G('remote_ip').value == 'oss-internal.aliyuncs.com') ? '1' : '2';
</script>
